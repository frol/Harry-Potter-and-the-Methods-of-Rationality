...
grep "nocopy" part_$CHAPTER_NUMBER.html | sed -r "s/^id='storytext'>(.*)/\1/;s/<br( [^>]*|\/)?>/<\/p><p>/g;s/ (<\/[^>]*>)/\1 /g;s/[\"]/ \"/g" | pandoc -f html -t latex | sed -r "s/\\\ldots\{\}/.../g;s/^\"/\`\`/g;s/\"/''/g;s/^ *-  */\"--* /g;s/([-,.?!])  *''/\1''/g;s/( |[,.?!] ?) *-  */\1\"--- /g" >> part_$CHAPTER_NUMBER.tex
